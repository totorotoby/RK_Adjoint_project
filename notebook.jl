...
# ╔═╡ dde8a3ad-7268-46b1-bc0d-b3b6b6ab5f63
begin
	...
	function linear_forward_euler(Δt, T, dx, pert, initials, conserve)
		t = 0:Δt:T
		N = length(t)
		x = zeros(N, 2)
		x[1, :] .= initials		 
		Q = zeros(N)
		for tstep in 2:N
			x[tstep, :] = x[tstep - 1, :] + Δt * (dx * x[tstep - 1, :])
		end
		return x, t, Q
	end
	...
	function plot_surf_solution(dx, x_low, x_high, y_low, y_high, T,
		...
		for step in sol_step:sol_step:sol_step*n_sol					
			x, t, Q = integrator(Δt, T, dx, step, initials, conserve)
			plt = scatter!(x[:,1], x[:,2], Q_func.(x[:,1], x[:2]), color=dcolor, markersize=.1, legend=false, markerstrokewidth=.01)
		end
		return plt
	end
end

# ╔═╡ 7586f6da-97b3-11ef-1d48-e55ef74909db
md"
## Symplectic Runge-Kutta Schemes for Adjoint Equations

This notebook is an attempt at working through the results/narrative of the paper J. M. Sanz-Serna, \"Symplectic Runge-Kutta schemes for adjoint equations, automatic differentiation, optimal control and more\".

The following 2D non-linear system of ODEs is used as an example:

``\dot x = -y (1 - x^2 - y^2)``

``\dot y = x (1 - x^2 - y^2)``

The additional Lotka–Volterra equations (a predator prey model) are also a useful example:
...
# ╔═╡ 9c41126c-07ce-4538-a649-7ca6c0968d06
begin
plot_surf_solution(f, -.2, .2, -.2, .2, 100,
...
# ╔═╡ 17cadf24-3c6c-4ac5-81fc-4c7533471a23
md"
## Sensitivity and adjoint equation
"

# ╔═╡ cd926102-03dc-467b-8e1d-3e1de9b78e48
begin

	Jg(x,y) = [1-y (-x) ;
			   (y) x-1]
	Gequal = [1, 1]
	Jge = Jg(Gequal[1], Gequal[2])

	x, t, Q = forward_euler(Δt, 2, g, .1, g_initials, H_func)
	δ, t, Q = linear_forward_euler(Δt, 2, Jge, .1, [.01, .01], H_func)
	plot(x[:, 1], x[:, 2])
	plot!(x[:, 1] .+ δ[:, 1], x[:,2] .+ x[:,2])
...
manifest_format = "2.0"
project_hash = "b81547a818fd4bda119e9c3f5b3fb63d4333fc8a"
